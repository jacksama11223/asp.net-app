...
- **Distributed Auth & State (CỰC KỲ QUAN TRỌNG):** 
    - Khi chạy Replicas > 1, BẮT BUỘC cấu hình `AddDataProtection().PersistKeysToStackExchangeRedis(...)`.
    - TUYỆT ĐỐI KHÔNG lưu cấu hình động (như tài khoản ngân hàng, settings) vào file cục bộ (JSON/XML) vì sẽ gây mất đồng bộ giữa các bản sao. Phải dùng **Redis** hoặc **Database**.
- **Nginx DNS & Route Resilience:** 
    - Khi thêm Controller MVC mới, phải khai báo Path vào `nginx-lb.conf`.
    - Sau khi rebuild Backend hoặc Frontend, PHẢI chạy lệnh `docker restart smartlms-lb` để cập nhật IP và Route mới.
